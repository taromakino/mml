...
def split_data(x, y, split_ratios):
    ...
    n_train, n_val, _ = [int(len(x) * split_ratio) for split_ratio in split_ratios]
...
def make_dataloaders(x_train, y_train, x_val, y_val, x_test, y_test, batch_size):
    data_train = DataLoader(TensorDataset(x_train, y_train), batch_size=batch_size, shuffle=True)
    data_val = DataLoader(TensorDataset(x_val, y_val), batch_size=batch_size, shuffle=True)
    data_test = DataLoader(TensorDataset(x_test, y_test), batch_size=batch_size, shuffle=True)
...
def vae_loss(x_reconst, x, mu, logvar):
    ...
    return reconst_loss + vae_kldiv(mu, logvar)

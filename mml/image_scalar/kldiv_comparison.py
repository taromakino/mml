...
def main(args):
    ...
    # Normalize
    x1_mean_s, x1_sd_s = x1_train_s.mean(), x1_train_s.std()
    x1_mean_ns, x1_sd_ns = x1_train_ns.mean(), x1_train_ns.std()

    x1_train_s = (x1_train_s - x1_mean_s) / x1_sd_s
    x1_val_s = (x1_val_s - x1_mean_s) / x1_sd_s
    x1_test_s = (x1_test_s - x1_mean_s) / x1_sd_s

    x1_train_ns = (x1_train_ns - x1_mean_ns) / x1_sd_ns
    x1_val_ns = (x1_val_ns - x1_mean_ns) / x1_sd_ns
    x1_test_ns = (x1_test_ns - x1_mean_ns) / x1_sd_ns

    # To torch
    x0_train_s, x1_train_s = torch.tensor(x0_train_s), torch.tensor(x1_train_s)[:, None]
    x0_val_s, x1_val_s = torch.tensor(x0_val_s), torch.tensor(x1_val_s)[:, None]
    x0_test_s, x1_test_s = torch.tensor(x0_test_s), torch.tensor(x1_test_s)[:, None]
    y_train_s, y_val_s, y_test_s = torch.tensor(y_train_s)[:, None], torch.tensor(y_val_s)[:, None], \
        torch.tensor(y_test_s)[:, None]

    x0_train_ns, x1_train_ns = torch.tensor(x0_train_ns), torch.tensor(x1_train_ns)[:, None]
    x0_val_ns, x1_val_ns = torch.tensor(x0_val_ns), torch.tensor(x1_val_ns)[:, None]
    x0_test_ns, x1_test_ns = torch.tensor(x0_test_ns), torch.tensor(x1_test_ns)[:, None]
    ...
    result = image_scalar_marginal_likelihood(data_test_ns, model_s, args.n_samples)
    write(test_fpath, f"model_s, data_test_ns, marginal_likelihood={result:.3f}")
    result = image_scalar_marginal_likelihood(data_test_s, model_ns, args.n_samples)
    write(test_fpath, f"model_ns, data_test_s, marginal_likelihood={result:.3f}")
    result = image_scalar_marginal_likelihood(data_test_ns, model_ns, args.n_samples)
